...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for KlerosGTCRAdapter RemoveRecipient transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async (_tcrItemId: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          KlerosGTCRAdapterContract.estimateGas.removeRecipient(_tcrItemId)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          KlerosGTCRAdapterContract.removeRecipient(_tcrItemId, { gasLimit: estimatedGas })
        );
