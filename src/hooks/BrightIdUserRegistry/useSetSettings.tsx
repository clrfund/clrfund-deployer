...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for BrightIdUserRegistry SetSettings transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_context:bytes32,_verifier:address,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_context:bytes32,_verifier:address,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useSetSettings(contractAddress:string){
  ...
  const handleSetSettings = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_context: string,_verifier: string,) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
            BrightIdUserRegistryContract.estimateGas.setSettings(_context,_verifier,)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          BrightIdUserRegistryContract.setSettings(_context,_verifier, { gasLimit: estimatedGas })
        );
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleSetSettings || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt,handleSetSettings,validator]);
