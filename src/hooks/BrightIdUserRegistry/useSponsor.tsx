...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for BrightIdUserRegistry Sponsor transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (addr:address,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (addr:address,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useSponsor(contractAddress:string){
    ...
    try {
      ...
      const sendTransaction = async (addr: string,) => {
        ...
        const { data: txObject, error: sendError } = await handle(
          BrightIdUserRegistryContract.sponsor(addr, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (addr:string,) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleSponsor || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt,handleSponsor,validator]);
