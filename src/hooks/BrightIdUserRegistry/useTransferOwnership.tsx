...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for BrightIdUserRegistry TransferOwnership transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async (newOwner: string,) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
            BrightIdUserRegistryContract.estimateGas.transferOwnership(newOwner,)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          BrightIdUserRegistryContract.transferOwnership(newOwner, { gasLimit: estimatedGas })
        );
