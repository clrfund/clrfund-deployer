...
export interface WalletModule {
  ...
  link?: string;
  installMessage?: (wallets: { currentWallet: string | undefined; selectedWallet: string }) => string;
  preferred?: boolean;
  desktop?: boolean;
  mobile?: boolean;
...
export interface Helpers {
  ...
  resetWalletState: (options?: { disconnected: boolean; walletName: string }) => void;
  browser: Browser;
  os: OS;
}
...
export interface WalletStateSliceStore {
  subscribe: (subscriber: (store: any) => void) => () => void;
  reset: () => void;
  setStateSyncer: (stateSyncer: StateSyncer) => { clear: () => void } | undefined;
  get: () => any;
}
