...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry ExecuteRequest transactions
 * - Example Response
 * {
      validator: {
...
function useExecuteRequest(contractAddress: string) {
    ...
    try {
      ...
      const sendTransaction = async (_recipientId: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.executeRequest(_recipientId)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.executeRequest(_recipientId, { gasLimit: estimatedGas })
        );
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleExecuteRequest || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt, handleExecuteRequest, validator]);
}
