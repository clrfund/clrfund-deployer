...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry ChallengeRequest transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_recipientId:bytes32,_beneficiary:address,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_recipientId:bytes32,_beneficiary:address,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useChallengeRequest(contractAddress: string) {
  ...
  const handleChallengeRequest = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_recipientId: string, _beneficiary: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.challengeRequest(_recipientId, _beneficiary)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.challengeRequest(_recipientId, _beneficiary, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_recipientId: string, _beneficiary: string) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleChallengeRequest || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt, handleChallengeRequest, validator]);
}

export default useChallengeRequest;
