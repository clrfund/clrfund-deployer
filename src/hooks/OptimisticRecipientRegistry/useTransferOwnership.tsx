...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry TransferOwnership transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async (newOwner: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.transferOwnership(newOwner)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.transferOwnership(newOwner, { gasLimit: estimatedGas })
        );
