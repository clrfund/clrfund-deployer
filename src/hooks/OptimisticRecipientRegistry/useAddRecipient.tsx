...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry AddRecipient transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_recipient:address,_metadata:string,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_recipient:address,_metadata:string,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useAddRecipient(contractAddress: string) {
  ...
  const handleAddRecipient = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_recipient: string, _metadata: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.addRecipient(_recipient, _metadata)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.addRecipient(_recipient, _metadata, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_recipient: string, _metadata: string) => {
      return true;
    };
