 ...
 *  @returns error || null
 */
function useDeployNewRound(contractAddress: string) {
    ...
    try {
      ...
      const sendTransaction = async () => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          FundingFactoryContract.estimateGas.deployNewRound()
        );
        ...
        const { data: txObject } = await handle(FundingFactoryContract.deployNewRound({ gasLimit: estimatedGas }));
        return txObject;
      };
  ...
  }, [library, account, chainId, getReceipt, handleDeployNewRound, validator]);
}
