 ...
 *
 *  @returns handleTransferMatchingFunds(_totalSpent:uint256,_totalSpentSalt:uint256,)
 *  @returns validator(_totalSpent:uint256,_totalSpentSalt:uint256,)
 *  @returns getReceipt(hash)
 *  @returns error || null
 */
function useTransferMatchingFunds(contractAddress: string) {
  ...
  const handleTransferMatchingFunds = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_totalSpent: string, _totalSpentSalt: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          FundingFactoryContract.estimateGas.transferMatchingFunds(_totalSpent, _totalSpentSalt)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          FundingFactoryContract.transferMatchingFunds(_totalSpent, _totalSpentSalt, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_totalSpent: string, _totalSpentSalt: string) => {
      return true;
    };
  ...
  }, [library, account, chainId, getReceipt, handleTransferMatchingFunds, validator]);
}
