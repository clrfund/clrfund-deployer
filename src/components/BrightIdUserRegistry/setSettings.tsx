...
import { useSetSettings } from "../../hooks/BrightIdUserRegistry/";
import { TransactionReceipt } from "../../hooks/BrightIdUserRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for setSettings method
 **/
export const SetSettingsForm = (props: any) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleSetSettings.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleSetSettings.error;

      const ok = await validator.checkArgs(data._context,data._verifier,);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleSetSettings.send(data._context,data._verifier,);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>SetSettings</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>SetSettings</Web3Form.Submit>
...
export default SetSettingsForm