...
import { useRemoveUser } from "../../hooks/SimpleUserRegistry/";
import { TransactionReceipt } from "../../hooks/SimpleUserRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for removeUser method
 **/
export const RemoveUserForm = (props: any) => {
  ...
  const { validator, handleRemoveUser, getReceipt, error } = useRemoveUser(contractAddress);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleRemoveUser.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleRemoveUser.error;

      const ok = await validator.checkArgs(data._user);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleRemoveUser.send(data._user);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>RemoveUser</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>RemoveUser</Web3Form.Submit>
