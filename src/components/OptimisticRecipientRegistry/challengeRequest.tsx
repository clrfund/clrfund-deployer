...
import { useChallengeRequest } from "../../hooks/OptimisticRecipientRegistry/";
import { TransactionReceipt } from "../../hooks/OptimisticRecipientRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for challengeRequest method
 **/
export const ChallengeRequestForm = (props: any) => {
  ...
  useEffect(() => {
    ...
    const contractAddress = query.get("OptimisticRecipientRegistryAddress");
    setContractAddress(contractAddress ? contractAddress : "0x0dA71825182944234F45755989a8C96Ac1343E07");
  }, [params]);

  const { validator, handleChallengeRequest, getReceipt, error } = useChallengeRequest(contractAddress);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleChallengeRequest.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleChallengeRequest.error;

      const ok = await validator.checkArgs(data._recipientId, data._beneficiary);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleChallengeRequest.send(data._recipientId, data._beneficiary);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>ChallengeRequest</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>ChallengeRequest</Web3Form.Submit>
