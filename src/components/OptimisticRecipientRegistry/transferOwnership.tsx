...
import { useTransferOwnership } from "../../hooks/OptimisticRecipientRegistry/";
import { TransactionReceipt } from "../../hooks/OptimisticRecipientRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for transferOwnership method
 **/
export const TransferOwnershipForm = (props: any) => {
  ...
  const { validator, handleTransferOwnership, getReceipt, error } = useTransferOwnership(contractAddress);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleTransferOwnership.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleTransferOwnership.error;

      const ok = await validator.checkArgs(data.newOwner);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleTransferOwnership.send(data.newOwner);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>TransferOwnership</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>TransferOwnership</Web3Form.Submit>
