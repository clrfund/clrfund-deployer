...
import { useSetBaseDeposit } from "../../hooks/OptimisticRecipientRegistry/";
import { TransactionReceipt } from "../../hooks/OptimisticRecipientRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for setBaseDeposit method
 **/
export const SetBaseDepositForm = (props: any) => {
  ...
  const { validator, handleSetBaseDeposit, getReceipt, error } = useSetBaseDeposit(contractAddress);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleSetBaseDeposit.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleSetBaseDeposit.error;

      const ok = await validator.checkArgs(data._baseDeposit);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleSetBaseDeposit.send(data._baseDeposit);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>SetBaseDeposit</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>SetBaseDeposit</Web3Form.Submit>
...
export default SetBaseDepositForm;
