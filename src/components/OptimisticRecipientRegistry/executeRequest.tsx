...
import { useExecuteRequest } from "../../hooks/OptimisticRecipientRegistry/";
import { TransactionReceipt } from "../../hooks/OptimisticRecipientRegistry/utils";
import { Web3Form } from "../Web3Form";
import { useLocation } from "react-router-dom";
/**
 * @class
 * @classdesc - component for executeRequest method
 **/
export const ExecuteRequestForm = (props: any) => {
  ...
  const { validator, handleExecuteRequest, getReceipt, error } = useExecuteRequest(contractAddress);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setTxLoading(true);
      if (validator.checkArgs == null || handleExecuteRequest.send == null || getReceipt.waitTwoBlocks == null)
        throw error ? error : handleExecuteRequest.error;

      const ok = await validator.checkArgs(data._recipientId);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleExecuteRequest.send(data._recipientId);
  ...
  return (
    <Web3Form.Form onSubmit={handleSubmit(onSubmit)}>
      <Web3Form.Title>ExecuteRequest</Web3Form.Title>
      ...
      <Web3Form.Input
        ...
        errors={errors}
      />
      <Web3Form.Submit loading={txLoading}>ExecuteRequest</Web3Form.Submit>
...
export default ExecuteRequestForm;
