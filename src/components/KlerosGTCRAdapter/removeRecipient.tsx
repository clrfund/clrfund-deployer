...
import { useRemoveRecipient } from "../../hooks/KlerosGTCRAdapter/";
...
export const RemoveRecipientForm = (props: any) => {
  ...
  useEffect(() => {
    ...
    const contractAddress = query.get("KlerosGTCRAdapterAddress");
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const ok = await validator.checkArgs(data._tcrItemId);
      if (!ok) throw Error("Failed smartcontract requirements");

      const tx = await handleRemoveRecipient.send(data._tcrItemId);
