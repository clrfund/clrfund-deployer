...
/**
 * @description ### Returns handler, validator, receipt, and error hooks for FundingFactory DeployNewRound transactions
 * - Example Response [handleDeployNewRound, validator, getReceipt, error]
 *
 *  @returns handleDeployNewRound()
 *  @returns validator() 
 *  @returns getReceipt(hash)
 *  @returns error || null
 */
function useDeployNewRound(contractAddress = "") {
  const { library, account, chainId } = useWeb3Context();

  const handleDeployNewRound = useMemo(() => {
    ...
    const sendTransaction = async () => {
      ...
      const estimatedGas = await FundingFactoryContract.estimateGas.deployNewRound().catch(() => {
        return FundingFactoryContract.estimateGas.deployNewRound();
      });
      //NOTE: tx object properties will be null except for tx.hash because we use UncheckedJsonRpcSigner
      return await handle(FundingFactoryContract.deployNewRound( { gasLimit: estimatedGas }));
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleDeployNewRound || !validator || !getReceipt
        ? Error("Missing dependencies, check your web3 provider")
        : null;

    return {
      validator,
      handleDeployNewRound,
      getReceipt,
      error,
    };
  }, [library, account, chainId]);
...
export default useDeployNewRound;