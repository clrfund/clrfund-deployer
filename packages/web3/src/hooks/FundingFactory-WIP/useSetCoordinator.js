 ...
 *
 *  @returns handleSetCoordinator(_coordinator,_coordinatorPubKey,)
 *  @returns validator(_coordinator,_coordinatorPubKey,) 
 *  @returns getReceipt(hash)
 *  @returns error || null
 */
function useSetCoordinator(contractAddress = "") {
  ...
  const handleSetCoordinator = useMemo(() => {
    ...
    const sendTransaction = async (_coordinator,_coordinatorPubKey,) => {
      //NOTE: Get the estimated gas price for this transaction
      const estimatedGas = await FundingFactoryContract.estimateGas.setCoordinator(_coordinator,_coordinatorPubKey,).catch(() => {
        return FundingFactoryContract.estimateGas.setCoordinator(_coordinator,_coordinatorPubKey,);
      });
      //NOTE: tx object properties will be null except for tx.hash because we use UncheckedJsonRpcSigner
      return await handle(FundingFactoryContract.setCoordinator(_coordinator,_coordinatorPubKey, { gasLimit: estimatedGas }));
    };
  ...
  const validator = useMemo(() => {
    ...
    // const [FundingFactoryContract, err] = getFundingFactoryContract(contractAddress, library, account);
    const validator = async (_coordinator,_coordinatorPubKey,) => {
      return true;
    };
    return validator;
  }, [library, account, chainId]);
