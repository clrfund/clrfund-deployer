 ...
 *
 *  @returns handleSetToken(_token,)
 *  @returns validator(_token,) 
 *  @returns getReceipt(hash)
 *  @returns error || null
 */
function useSetToken(contractAddress = "") {
  ...
  const handleSetToken = useMemo(() => {
    ...
    const sendTransaction = async (_token,) => {
      //NOTE: Get the estimated gas price for this transaction
      const estimatedGas = await FundingFactoryContract.estimateGas.setToken(_token,).catch(() => {
        return FundingFactoryContract.estimateGas.setToken(_token,);
      });
      //NOTE: tx object properties will be null except for tx.hash because we use UncheckedJsonRpcSigner
      return await handle(FundingFactoryContract.setToken(_token, { gasLimit: estimatedGas }));
    };
