...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for MACIFactory RenounceOwnership transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async () => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          MACIFactoryContract.estimateGas.renounceOwnership()
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          MACIFactoryContract.renounceOwnership({ gasLimit: estimatedGas })
        );
