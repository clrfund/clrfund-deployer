...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for SimpleUserRegistry TransferOwnership transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async (newOwner: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          SimpleUserRegistryContract.estimateGas.transferOwnership(newOwner)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          SimpleUserRegistryContract.transferOwnership(newOwner, { gasLimit: estimatedGas })
        );
