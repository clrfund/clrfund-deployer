...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for SimpleUserRegistry RemoveUser transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_user:address,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_user:address,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useRemoveUser(contractAddress: string) {
    ...
    try {
      ...
      const sendTransaction = async (_user: string) => {
        ...
        const { data: txObject, error: sendError } = await handle(
          SimpleUserRegistryContract.removeUser(_user, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_user: string) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleRemoveUser || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt, handleRemoveUser, validator]);
}
