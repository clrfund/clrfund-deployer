...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for SimpleUserRegistry AddUser transactions
 * - Example Response
 * {
      validator: {
 ...
 *   }
 */
function useAddUser(contractAddress: string) {
    ...
    try {
      ...
      const sendTransaction = async (_user: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          SimpleUserRegistryContract.estimateGas.addUser(_user)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          SimpleUserRegistryContract.addUser(_user, { gasLimit: estimatedGas })
        );
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleAddUser || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt, handleAddUser, validator]);
}
