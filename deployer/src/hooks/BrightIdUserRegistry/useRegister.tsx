...
import { getBrightIdUserRegistryContract, handle, TransactionReceiptOrError } from "./utils";
import { useWeb3Context } from "../Web3Provider";

/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for BrightIdUserRegistry Register transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_context:bytes32,_addrs:address[],_timestamp:uint256,_v:uint8,_r:bytes32,_s:bytes32,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_context:bytes32,_addrs:address[],_timestamp:uint256,_v:uint8,_r:bytes32,_s:bytes32,) => Promise<...>;
        error: null;
      }
      getReceipt: {
 ...
 *   }
 */
function useRegister(contractAddress:string){
  ...
  const handleRegister = useMemo(() => {
    ...
    const { contract: BrightIdUserRegistryContract, error } = getBrightIdUserRegistryContract(contractAddress, library, account);

    try {
      if (error || BrightIdUserRegistryContract == null)
        throw Error("failed to get BrightIdUserRegistry Contract at " + contractAddress);

      const sendTransaction = async (_context: string,_addrs: string,_timestamp: string,_v: string,_r: string,_s: string,) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
            BrightIdUserRegistryContract.estimateGas.register(_context,_addrs,_timestamp,_v,_r,_s,)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          BrightIdUserRegistryContract.register(_context,_addrs,_timestamp,_v,_r,_s, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_context:string,_addrs:string,_timestamp:string,_v:string,_r:string,_s:string,) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleRegister || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt,handleRegister,validator]);
