...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry RenounceOwnership transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async () => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.renounceOwnership()
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.renounceOwnership({ gasLimit: estimatedGas })
        );
