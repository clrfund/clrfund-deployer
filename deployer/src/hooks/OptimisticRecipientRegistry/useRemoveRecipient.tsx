...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry RemoveRecipient transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_recipientId:bytes32,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_recipientId:bytes32,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useRemoveRecipient(contractAddress: string) {
  ...
  const handleRemoveRecipient = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_recipientId: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.removeRecipient(_recipientId)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.removeRecipient(_recipientId, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_recipientId: string) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleRemoveRecipient || !validator || !getReceipt
  ...
  }, [library, account, chainId, getReceipt, handleRemoveRecipient, validator]);
}

export default useRemoveRecipient;
