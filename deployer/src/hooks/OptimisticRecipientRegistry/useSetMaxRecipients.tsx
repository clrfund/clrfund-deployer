...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry SetMaxRecipients transactions
 * - Example Response
 * {
      validator: {
    ...
    try {
      ...
      const sendTransaction = async (_maxRecipients: string) => {
        ...
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.setMaxRecipients(_maxRecipients)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.setMaxRecipients(_maxRecipients, { gasLimit: estimatedGas })
        );
