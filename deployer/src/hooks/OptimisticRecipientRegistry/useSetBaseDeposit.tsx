...
/**
 * @description ### Returns  validator,handler, getReceipt, and error hooks for OptimisticRecipientRegistry SetBaseDeposit transactions
 * - Example Response
 * {
      validator: {
        checkArgs: (_baseDeposit:uint256,) => Promise<boolean>;
        error: null;
      }
      handleAddFundingSource: {
        send: (_baseDeposit:uint256,) => Promise<...>;
        error: null;
      }
      getReceipt: {
...
function useSetBaseDeposit(contractAddress: string) {
  ...
  const handleSetBaseDeposit = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_baseDeposit: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          OptimisticRecipientRegistryContract.estimateGas.setBaseDeposit(_baseDeposit)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          OptimisticRecipientRegistryContract.setBaseDeposit(_baseDeposit, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_baseDeposit: string) => {
      return true;
    };
  ...
  return useMemo(() => {
    const error =
      !library || !account || !chainId || !handleSetBaseDeposit || !validator || !getReceipt
        ? Error("Missing dependencies, check your web3 provider")
        : null;

    return {
      validator,
      handleSetBaseDeposit,
      getReceipt,
      error,
    };
  }, [library, account, chainId, getReceipt, handleSetBaseDeposit, validator]);
}

export default useSetBaseDeposit;
