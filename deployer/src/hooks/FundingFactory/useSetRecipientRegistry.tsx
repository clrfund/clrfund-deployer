 ...
 *
 *  @returns handleSetRecipientRegistry(_recipientRegistry:address,)
 *  @returns validator(_recipientRegistry:address,)
 *  @returns getReceipt(hash)
 *  @returns error || null
 */
function useSetRecipientRegistry(contractAddress: string) {
  ...
  const handleSetRecipientRegistry = useMemo(() => {
    ...
    try {
      ...
      const sendTransaction = async (_recipientRegistry: string) => {
        //NOTE: Get the estimated gas price for this transaction
        const { data: estimatedGas, error: gasEstimateError } = await handle(
          FundingFactoryContract.estimateGas.setRecipientRegistry(_recipientRegistry)
        );
        ...
        const { data: txObject, error: sendError } = await handle(
          FundingFactoryContract.setRecipientRegistry(_recipientRegistry, { gasLimit: estimatedGas })
        );
  ...
  const validator = useMemo(() => {
    ...
    const checkArgs = async (_recipientRegistry: string) => {
      return true;
    };
  ...
  }, [library, account, chainId, getReceipt, handleSetRecipientRegistry, validator]);
}
